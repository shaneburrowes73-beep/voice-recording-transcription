...
export default function DataStorage({ user }) {
  ...
  const exportCSV = () => {
    ...
    const a=document.createElement("a"); a.href=URL.createObjectURL(new Blob([csv],{type:"text/csv"})); a.download=`transcripts-${new Date().toISOString().slice(0,10)}.csv`; a.click();
  };

  const syncFromSheets = async () => {
    setSyncing(true); setSyncMsg("");
    const remote = await fetchFromSheets(user.officer, user.role);
    if(remote&&remote.length){ dispatch({type:"LOAD_TRANSCRIPTS",payload:remote}); setSyncMsg(`â Synced ${remote.length} records`); }
  ...
  const doEmail = async () => {
    ...
    setEmailMsg("â Email sent via Gmail");
  ...
  return (
    <div>
      <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1rem" }}>
        <h2 style={{ margin:0,fontSize:16,fontWeight:600 }}>ð Data Storage & Retrieval</h2>
      ...
      <div style={{ display:"flex",gap:8,marginBottom:10,flexWrap:"wrap" }}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search transcripts, officers, notesâ¦" style={{ ...inp(),flex:2,minWidth:140 }}/>
      ...
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10 }}>
        ...
        <div style={{ display:"flex",gap:8 }}>
          <button onClick={syncFromSheets} disabled={syncing} style={{ ...btn(),fontSize:12,padding:"5px 12px" }}>{syncing?"Syncingâ¦":"â» Sync"}</button>
          <button onClick={exportCSV} style={{ ...btn(),fontSize:12,padding:"5px 12px" }}>â¬ CSV</button>
        </div>
      </div>
          ...
          return (
            <div key={t.id} style={{ ...card({cursor:"pointer"}) }} onClick={()=>setSelected(isOpen?null:t)}>
              <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6 }}>
                <div style={{ display:"flex",alignItems:"center",gap:8 }}><Avatar name={t.officer} size={32}/><div><div style={{ fontSize:13,fontWeight:600 }}>{t.officer}</div><div style={{ fontSize:11,color:"#6b7280" }}>{t.role} Â· {t.timestamp}</div></div></div>
                <Tag color={sc} label={t.status}/>
              </div>
              <p style={{ margin:0,fontSize:13,color:"#6b7280",overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis" }}>{txt.slice(0,140)}{txt.length>140?"â¦":""}</p>
              {isOpen&&(
                <div onClick={e=>e.stopPropagation()} style={{ marginTop:12 }}>
                  ...
                  {t.reviewNote&&<div style={{ background:C.amber.bg,border:`1px solid ${C.amber.border}`,borderRadius:8,padding:"8px 12px",marginBottom:10 }}><div style={{ fontSize:11,color:C.amber.text }}>Review â {t.reviewedBy} Â· {t.reviewedAt}</div><p style={{ margin:"4px 0 0",fontSize:13 }}>{t.reviewNote}</p></div>}
                  ...
                  <div style={{ ...card({background:"#f0f9ff",border:`1px solid ${C.blue.border}`}) }}>
                    <div style={{ fontSize:13,fontWeight:600,marginBottom:8 }}>â Send via Gmail</div>
                    ...
                    <button onClick={doEmail} disabled={sendingEmail||!emailTo.trim()} style={{ ...btn("primary"),width:"100%" }}>{sendingEmail?"Sendingâ¦":"Send email"}</button>
