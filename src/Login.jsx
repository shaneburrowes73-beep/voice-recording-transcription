...
export default function LoginPage(){
  ...
  useEffect(()=>{fetch('/api/roles').then(r=>r.json()).then(d=>{if(d.roles)setRoles(d.roles.map(r=>r.name));}).catch(()=>{});},[] );
  ...
  const handleSubmit=async()=>{
    ...
    try{
      if(mode==='login'){await login(form.email,form.password);}
        ...
        await register({fullName:form.fullName,badgeNo:form.badgeNo,email:form.email,password:form.password,role:form.role});
        setSuccess('Account created! You can now sign in.');setMode('login');
  ...
  return(
    <div style={{maxWidth:420,margin:'0 auto',padding:'0 1rem',fontFamily:'system-ui,sans-serif',minHeight:'100vh',display:'flex',flexDirection:'column',justifyContent:'center'}}>
      ...
      <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:16,padding:'1.25rem'}}>
        ...
        {mode==='register'&&(
          <div style={{background:'#fef3c7',border:'1px solid #fcd34d',borderRadius:8,padding:'8px 12px',marginBottom:14}}>
            <p style={{fontSize:12,color:'#92400e',margin:0}}>⚠ Never share your password. This account is linked to your badge number and all activity is logged.</p>
          </div>
        )}
