...
export default function VoiceCapture({ user, onSaved }) {
  ...
  useEffect(() => {
    if (!('webkitSpeechRecognition' in window || 'SpeechRecognition' in window)) setSpeechOk(false);
  }, []);
        ...
        <div style={{background:P,padding:'0 16px 16px'}}>
          <div style={{height:44,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            <div style={{display:'flex',alignItems:'center',gap:8}}>
              <div style={{width:32,height:22,borderRadius:3,overflow:'hidden',display:'flex',flexShrink:0}}>
                <div style={{flex:1,background:P,border:'1px solid rgba(255,255,255,0.3)'}}/><div style={{flex:1,background:A,display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,color:'#000'}}>{brand.trident}</div><div style={{flex:1,background:P,border:'1px solid rgba(255,255,255,0.3)'}}/>
              </div>
              <div>
          ...
          {phase==='idle'&&(
            <div style={{background:'rgba(255,255,255,0.12)',borderRadius:12,padding:'10px 14px',display:'flex',alignItems:'center',gap:10}}>
              ...
              <div style={{flex:1}}><div style={{color:'#fff',fontSize:14,fontWeight:500}}>{user.officer}</div><div style={{color:'rgba(255,255,255,0.75)',fontSize:12}}>{user.role}</div></div>
              <div style={{background:A,color:AT,fontSize:10,fontWeight:700,padding:'2px 8px',borderRadius:20}}>{user.badgeNo||'—'}</div>
            </div>
          )}
          {phase!=='idle'&&<div style={{color:'rgba(255,255,255,0.85)',fontSize:13,textAlign:'center',paddingTop:4}}>{phase==='recording'?'Recording in progress…':phase==='stopped'?'Recording stopped':phase==='reviewing'?'Review corrected report':'Review your recording'}</div>}
        </div>
        <div style={{height:5,display:'flex'}}>{brand.flagStripe.map((c,i)=><div key={i} style={{flex:1,background:c}}/>)}</div>
        <div style={{flex:1,padding:'16px',overflowY:'auto'}}>
          ...
          {phase==='idle'&&(<>
            <div style={{textAlign:'center',margin:'8px 0 20px'}}>
              <div style={{width:110,height:110,borderRadius:'50%',background:P,display:'inline-flex',alignItems:'center',justifyContent:'center',cursor:'pointer',boxShadow:`0 0 0 16px rgba(0,38,127,0.08),0 0 0 28px rgba(0,38,127,0.04)`}} onClick={startRec}>
          ...
          {phase==='reviewing'&&(<>
            <div style={{...card({marginBottom:12}),border:'1px solid #86efac',background:'#f0fdf4'}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:6}}><div style={{fontSize:11,color:'#9ca3af',textTransform:'uppercase',letterSpacing:'0.04em'}}>Corrected transcript</div><span style={{background:'#d1fae5',color:'#065f46',fontSize:10,fontWeight:600,padding:'2px 8px',borderRadius:20}}>AI corrected</span></div>
