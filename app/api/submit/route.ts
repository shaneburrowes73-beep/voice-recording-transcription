...
export async function POST(req: Request) {
  try {
    ...
    // Fire transcription async — calls Whisper directly, no self-HTTP
    transcribeSubmission(submissionId, p.audioFileUrls).catch(err =>
      console.error('Background transcription error:', err?.message || err)
    );
